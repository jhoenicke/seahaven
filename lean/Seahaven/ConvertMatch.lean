...
/-! `convertFromPilesKings`'s simulation from the entry state — `cvCleanupLoop_lax`,
`convert_simulates_lax`, `solve_correct_lax` — moved to `CleanupLax`: `CvCleanupSim`'s
only proof, `cvCleanupSim`, is discharged there, and these no longer take it as a
hypothesis, so they must live downstream of that proof rather than upstream of it. -/
