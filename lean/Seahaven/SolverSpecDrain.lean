...
`rank`, the termination measure for the `while busyAces ≠ 0 do moveAces
...
private def rank (game : PosType) : Nat :=
...
/-- **`rank` strictly decreases across one `moveAces_merged` step.**  Uses
    ...
    drops. -/
private theorem rank_decrease (g : Globals) (game game1 : PosType)
...
/-- What the drain loop hands to a carried predicate: one `moveAces` call from a
...
def DrainStep (g : Globals) (P : UInt16 → PosType → Prop) : Prop :=
  ∀ (fkAcc fk : UInt16) (game game1 : PosType),
    SolverInvMerged g game → game.busyAces ≠ 0 →
    Solver.moveAces (g, game) = .ok fk (g, game1) →
...
private theorem drainBody_run (g : Globals) (hwf : WellFormedLayout g)
    (P : UInt16 → PosType → Prop) (hstep : DrainStep g P) :
    ∀ (n : Nat) (forcedKings : UInt16) (game : PosType),
      rank game < n →
      SolverInvMerged g game →
      P forcedKings game →
      ∃ (forcedKings' : UInt16) (game' : PosType),
  ...
  | succ n ih =>
    ...
    · obtain ⟨fk, game1, hrun1, hmerged1, hframe1, hdich1, -⟩ :=
      ...
      have hrun1' : Solver.moveAces (g, game) = .ok fk (g, game1) := hrun1
...
/-- **The drain loop reaches canonical form.**  From a merged state, draining
    `busyAces` via the real `while busyAces ≠ 0 do moveAces()` loop
    (`drainBody`, shared by `move` and `convertFromPilesKings`)
    reaches a fully canonical state. -/
theorem drain_canonical (g : Globals) (p : PosType) (fk0 : UInt16)
  ...
  obtain ⟨fk, p', hrun, hmerged', hbz, hle⟩ :=
    drainBody_run g hwf (fun _ game => DepthLe p game)
      (by
        -- each iteration is one `moveAces`, and those never deepen a pile
        intro _fkAcc fk game game1 hm hbz hrunMA hP
        obtain ⟨fk2, p2, hrun2, -, -, -, hle2⟩ := moveAces_merged g game hwf hm hbz
        have hrunMA' : EStateM.run Solver.moveAces (g, game) = .ok fk (g, game1) := hrunMA
...
/-- **The drain loop, carrying a predicate across each `moveAces` call.**  Same
...
theorem drain_canonical_of (g : Globals) (p : PosType) (fk0 : UInt16)
    (hwf : WellFormedLayout g) (hmerged : SolverInvMerged g p)
    (P : UInt16 → PosType → Prop) (hstep : DrainStep g P) (hP : P fk0 p) :
    ∃ fk p', Loop.forIn Loop.mk fk0 drainBody (g, p) = .ok fk (g, p') ∧
